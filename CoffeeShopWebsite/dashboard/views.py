...
class OrderDetailView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        initial_data = model_to_dict(
            item, fields=[field.name for field in item._meta.fields]
        )
        form = self.form_class(initial=initial_data)
        cafeitems = CafeItem.objects.all()

        context = {"order": item, "form": form, "cafeitems": cafeitems}
            ...
            if item.isdigit() and quantity.isdigit():
                if OrderItem.objects.filter(order_id=kwargs["pk"], cafeitem_id=item):
                    ...
                    OrderItem.objects.create(
                        order_id=kwargs["pk"], cafeitem_id=item, quantity=int(quantity)
                    )
...
class DashboardView(View, PermissionRequiredMixin):
...
def month_filter_options(request):
    grouped_orders = (
        Order.objects.annotate(day=ExtractMonth("order_date"))
        .values("day")
        .order_by("-day")
        .distinct()
    )
...
def day_filter_options(request):
    grouped_orders = (
        Order.objects.annotate(hour=ExtractHour("order_date"))
        .values("hour")
        .order_by("-hour")
        .distinct()
    )
...
def monthly_sales_chart(request):
    ...
    grouped_orders = (
        orders.annotate(p=F("price"))
        .annotate(day=ExtractDay("order__order_date"))
        .values("day")
        .annotate(total=Sum("price"))
        .values("day", "total")
        .order_by("day")
...
def daily_sales_chart(request):
    ...
    grouped_orders = (
        orders.annotate(p=F("price"))
        .annotate(hour=ExtractHour("order__order_date"))
        .values("hour")
        .annotate(total=Sum("price"))
        .values("hour", "total")
        .order_by("hour")
...
def sales_by_time_of_day(request):
    ...
    time_sales = (
        ...
        .values("hour")
        .annotate(total=Sum("price"))
        .values("order__order_date__date", "hour", "total")
    )

    new_list = list()
    for d in time_sales:
        new_dict = {
            "datetime": str(d["order__order_date__date"])
            + " "
            + str(d["hour"])
            + ":00:00",
            "total": d["total"],
    ...
    date_list = [
        {"datetime": date, "total": price} for date, price in new_dicts.items()
    ]
...
def total_sales(request):
    ...
    all_orders_in_range = (
        orders.annotate(p=F("price")).annotate(total=Sum("price")).values("total")
    )
...
def top_10_selling_items(request):  # year, month, day
    ...
    chart_items = [
        {"cafeitem__name": name, "total": price} for name, price in new_dict.items()
    ]
...
def top_10_customers(requests):
    ...
    all_numbers = (
        orders.annotate(p=F("price"))
        .annotate(total=Sum("price"))
        .values("order__phone_number", "total")
    )
    ...
    chart_ph_numbers = [
        {"order__phone_number": number, "total": price}
        for number, price in new_dict.items()
    ]
...
def sales_by_category(requests):
    ...
    all_caterories = (
        orders.annotate(p=F("price"))
        .annotate(total=Sum("price"))
        .values("cafeitem__category__name", "total")
    )
    ...
    chart_category = [
        {"cafeitem__category__name": name, "total": price}
        for name, price in new_dict.items()
    ]
...
def sales_by_employee(request):  # Table, or a bar Chart.
    ...
    all_staff = (
        orders.annotate(p=F("price"))
        .annotate(total=Sum("price"))
        .values("order__staff__first_name", "order__staff__last_name", "total")
    )
    ...
    for d in all_staff:
        try:
            new_dict = {
                "staff_name": d["order__staff__first_name"]
                + " "
                + d["order__staff__last_name"],
                "total": d["total"],
            }
            new_list.append(new_dict)
        except:
    ...
    staff_list = [
        {"staff_name": name, "total": price} for name, price in new_dicts.items()
    ]
...
def order_status_report(
    request, status: str
):  # Table, not a Chart. status= "D", "C", "A"
    date1 = request.GET.get("start_date", None)
    date2 = request.GET.get("end_date", None)
...
def ranking(phone_number: str):
    ...
    all_orders = (
        all_customers.annotate(p=F("price"))
        .annotate(total=Sum("price"))
        .values("order__phone_number", "total")
    )
    ...
    ranking_list = [
        {"order__phone_number": number, "total": total}
        for number, total in new_dicts.items()
    ]
...
def customer_data(request):
    ...
    # Favorite item
    all_orders_quantity = (
        orders.annotate(p=F("quantity"))
        .annotate(quantity=Sum("quantity"))
        .values("cafeitem__name", "quantity")
    )
    ...
    item_quantity_list = [
        {"cafeitem__name": name, "quantity": quantity}
        for name, quantity in new_dicti.items()
    ]
    sorted_item_quantity_list = sorted(
        item_quantity_list, key=lambda x: x["quantity"], reverse=True
    )
    ...
    all_orders_category = (
        orders.annotate(p=F("quantity"))
        .annotate(quantity=Sum("quantity"))
        .values("cafeitem__category__name", "quantity")
    )
    ...
    category_quantity_list = [
        {"cafeitem__category__name": name, "quantity": quantity}
        for name, quantity in new_dictc.items()
    ]
    sorted_category_quantity_list = sorted(
        category_quantity_list, key=lambda x: x["quantity"], reverse=True
    )
    ...
    # Customer attendance
    attended_orders = Order.objects.filter(phone_number=phone)
...
def number_of_items_bought(request):
    ...
    all_orders = (
        orders.annotate(p=F("quantity"))
        .annotate(number=Sum("quantity"))
        .values("cafeitem__name", "number")
    )
