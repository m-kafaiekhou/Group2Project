...
class DashboardTests(TestCase):
    ...
    def test_order_detail_view_post(self):
        ...
        response = self.client.post(
            reverse("order_details", kwargs={"pk": self.order.pk}), data=data
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("order_details", kwargs={"pk": self.order.pk})
        )
    ...
    def test_order_list_view_post(self):  # Not Done
        ...
        response = self.client.post(
            reverse("order_status", kwargs={"stat": self.order.status}), data=data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_order_status(self):
        self.client.login(phone_number="09030001122", password="1X<ISRUkw+tuK")
        response = self.client.get(
            reverse("order_status", kwargs={"stat": self.order.status})
        )
        self.assertEqual(response.status_code, 200)
